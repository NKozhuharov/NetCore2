<?php
if (!class_exists("mc")) {
    throw new Error("No Memcached Class Defined");
}
...
class db
{
    ...
    public static function getInstance($Core=false){
        if(self::$instance==false && $Core==false && self::$Core==false)
            throw new Error("Not connected to MYSLQ");
    ...
    function selectConnect(){
        if($this->select == false){
            ...
            if(@!is_object($this->select) || @!empty($this->select->error) || !empty($this->select->connect_error)){
                throw new Error('Mysql server is overloaded!', 1001);
            }
        ...
        if($select){
            ...
            if($newQuery){
                ...
                if(is_object($query)){
                    ...
                    throw new Error($Core->generalErrorText);
                }
            }else{
                if(!empty($var['query'])){
            ...
            if($this->insert!=false){
                ...
                if(isset($error))
                    if(!defined("noExceptionOnDuplicate") || !stristr($this->insert->error,'Duplicate entry'))
                        throw new Error("Mysql Error: ".$this->insert->error);
                ...
                throw new Error("Insert is prohibited at the current moment.");
            }
        }
    }
